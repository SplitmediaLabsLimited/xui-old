...
describe('xui.core.App', function() {
  ...
  describe ('should get audio devices', function() {
          ...
          setTimeout(function() {
            window.OnAsyncCallback(randomNumber,
              encodeURIComponent('<devices>' +
                '<dev id="empty" level="1.000000" enable="1"' +
    ...
    it('through a promise', function() {
      var promise = App.getAudioDevices();
      expect(promise).toBeInstanceOf(Promise);
    });
    ...
    it('that returns an array of xui.system.Audio', function(done) {
      ...
      promise.then(function(audios) {
        ...
        expect(audios)
          .eachHasMethods('setLevel, getLevel, isEnabled, setEnabled,' +
            ' getSystemLevel, setSystemLevel, isSystemEnabled,' +
            ' setSystemEnabled, getDelay, setDelay');
        done();
      });
    });
  });
  ...
  describe ('should be able to set audio devices', function() {
    ...
    it('as a string', function(done) {
      ...
      var audioDevices = parseXml(decodeURIComponent(micDev2Mock))
        .getElementsByTagName('dev');
      var audioDevicesArray = [];
      for (var i = 0; i < audioDevices.length; i++) {
        audioDevicesArray.push(
          new audioDevice(audioDevices[i].getAttribute('id'),
            audioDevices[i].getAttribute('level'),
            audioDevices[i].getAttribute('enable'),
            audioDevices[i].getAttribute('hwlevel'),
            audioDevices[i].getAttribute('hwenable'),
            audioDevices[i].getAttribute('delay')));
      };
      App.setAudioDevices(audioDevicesArray);
      expect(micDev2Set).toBe(true);
      done();
    });
  });
  ...
  describe('should get presentation', function() {
    beforeEach(function() {
      spyOn(window.external, 'AppGetPropertyAsync')
        .and.callFake(function(funcName) {
        if (funcName == 'version') {
  ...
  describe ('should be able to save the current presentation', function() {
    var saveSet = false;
    beforeEach(function() {
      spyOn(window.external, 'AppCallFuncAsync')
        .and.callFake(function(funcName, value) {
